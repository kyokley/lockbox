...
def cli_encrypt(
    ...
    if data:
        ...
        if infile.is_file():
            encrypt_file(
                passphrase, infile, output_file=outfile, remove_original=remove_original
            )
        elif infile.is_dir():
            if not recursive:
                raise LockBoxException(
                    "{} is a directory but --recursive has not been specified".format(
                        infile
                    )
                )
            else:
...
def _validate_files(infile, outfile, force):
    ...
    if outfile and not force and outfile.exists():
        print(term.yellow(f"WARNING: {outfile} already exists\n"))
        confirm = input(
            term.yellow(f"Are you absolutely sure you want to delete {outfile}? [y/N] ")
        )
        ...
        if infile.is_file():
            if remove_original and not force:
                if not outfile:
                    print(
                        term.red(
                            "WARNING: No output file has been specified. After removing original, data will only be displayed on stdout\n"
                        )
                    )
                confirm = input(
                    term.yellow(
                        "Are you absolutely sure you want to delete {} after decryption is complete? [y/N] ".format(
                            infile
                        )
                    )
                )
