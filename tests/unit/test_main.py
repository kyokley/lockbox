...
class TestEncrypt:
    ...
    def test_password_is_str(self):
        ...
        self.mock_get_fernet.assert_called_once_with(
            b"test_password", self.mock_token_bytes.return_value
        )
        self.mock_get_fernet.return_value.encrypt.assert_called_once_with(
            self.plaintext
        )
        self.mock_urlsafe_b64encode.assert_called_once_with(
            self.mock_token_bytes.return_value
        )
    ...
    def test_data_is_str(self):
        ...
        self.mock_token_bytes.assert_called_once_with(SALT_LENGTH)
        self.mock_get_fernet.assert_called_once_with(
            self.password, self.mock_token_bytes.return_value
        )
    ...
    def test_outfile_no_qr_code(self):
        expected = None
        actual = encrypt(
            self.password, self.plaintext, outfile=Path("test_outfile.txt")
        )
        ...
        self.mock_open.return_value.write.assert_called_once_with(
            b"test_encoded_salt$test_cipher_data"
        )

    def test_outfile_qr_code(self):
        expected = None
        actual = encrypt(
            self.password, self.plaintext, outfile=Path("test_outfile.png")
        )
        ...
        self.mock_qrcode.make.assert_called_once_with(
            b"test_encoded_salt$test_cipher_data"
        )
        self.mock_qrcode.make.return_value.save.assert_called_once_with(
            Path("test_outfile.png")
        )
...
class TestDecrypt:
    ...
    def test_password_is_str_raises(self):
        ...
        self.mock_get_fernet.assert_called_once_with(
            self.password, b"test_decoded_salt"
        )
